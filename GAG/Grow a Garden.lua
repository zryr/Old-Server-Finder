...
local function createMainGui()
    ...
    titleLabel.Text = "Choose an Option"; titleLabel.Size = UDim2.new(1, 0, 0, 35)
    ...
    versionLabel.BackgroundTransparency = 1; versionLabel.TextColor3 = Color3.fromRGB(180,180,180)
    versionLabel.TextXAlignment = Enum.TextXAlignment.Left; versionLabel.Font = Enum.Font.SourceSans; versionLabel.TextSize = 14

    versionInputBox = Instance.new("TextBox", frame)
    versionInputBox.Size = UDim2.new(0.85, 0, 0, 35)
    versionInputBox.Position = UDim2.new(0.5, 0, 0, 65); versionInputBox.AnchorPoint = Vector2.new(0.5, 0)
    versionInputBox.BackgroundColor3 = Color3.fromRGB(30,30,30); versionInputBox.TextColor3 = Color3.fromRGB(220,220,220)
    versionInputBox.PlaceholderText = "e.g. " .. DEFAULT_TARGET_MAX_VERSION
    versionInputBox.Text = tostring(CURRENT_TARGET_MAX_VERSION) 
    ...
    statusLabel.Position = UDim2.new(0.5, 0, 0, 105); statusLabel.AnchorPoint = Vector2.new(0.5, 0)
    ...
    local searchButton = createPillButton(frame, "Search Old Server", UDim2.new(0.5, 0, 0.65, 0), UDim2.new(0.85, 0, 0, 45))
    searchButton.AnchorPoint = Vector2.new(0.5, 0.5)

    local joinButton = createPillButton(frame, "Join Old Server", UDim2.new(0.5, 0, 0.85, 0), UDim2.new(0.85, 0, 0, 45))
    ...
    local function updateTargetVersionFromInput()
        ...
        if num and num > 0 then
            ...
            quickNotify("Target version set to: " .. CURRENT_TARGET_MAX_VERSION, 3)
            if statusLabel and statusLabel.Parent then statusLabel.Text = "Target <= V" .. CURRENT_TARGET_MAX_VERSION .. ". Select action." end
        else
            quickNotify("Invalid target version. Using: " .. CURRENT_TARGET_MAX_VERSION, 3)
            versionInputBox.Text = tostring(CURRENT_TARGET_MAX_VERSION) 
        end
    end
    ...
    searchButton.MouseButton1Click:Connect(function()
        updateTargetVersionFromInput() 
        if mainGui and mainGui.Parent then mainGui:Destroy() end
        quickNotify("Starting search for servers <= V" .. CURRENT_TARGET_MAX_VERSION, 3)
...
local function createPastebinPaste(name, content)
    ...
    if ACQUIRED_API_USER_KEY and ACQUIRED_API_USER_KEY ~= "" and not ACQUIRED_API_USER_KEY:find("YOUR_") then
...
local function deletePastebinPaste(pasteKeyToDelete)
    ...
    if PASTEBIN_API_DEV_KEY == "" or PASTEBIN_API_DEV_KEY:find("YOUR_") then quickNotify("Pastebin Dev Key not configured for delete!", 5); return false end
    if not ACQUIRED_API_USER_KEY or ACQUIRED_API_USER_KEY == "" or ACQUIRED_API_USER_KEY:find("YOUR_") then
        print("Cannot delete paste " .. pasteKeyToDelete .. ": api_user_key not available/configured.")
        return false
    end
...
local function updatePastebinList(listType, newDataToAdd)
    ...
    if newKey then
        ...
        if oldKey and oldKey ~= newKey and ACQUIRED_API_USER_KEY and ACQUIRED_API_USER_KEY ~= "" and not ACQUIRED_API_USER_KEY:find("YOUR_") then
...
local function mainEntryPoint() 
    ...
    if PASTEBIN_API_DEV_KEY == "" or PASTEBIN_API_DEV_KEY:find("YOUR_") then configError = true; configErrorMsg = configErrorMsg .. "Set PASTEBIN_API_DEV_KEY. " end
    if ACQUIRED_API_USER_KEY == "" or ACQUIRED_API_USER_KEY:find("YOUR_") then 
        configError = true; configErrorMsg = configErrorMsg .. "Set ACQUIRED_API_USER_KEY. "
    end
    if SCRIPT_RELOAD_URL == "" or SCRIPT_RELOAD_URL:find("YOUR_RAW_URL") then 
        print("INFO: SCRIPT_RELOAD_URL is using a placeholder or is empty. For queue_on_teleport to reload THIS script from a URL, update this value. Otherwise, it relies on auto-execute.")
    end
